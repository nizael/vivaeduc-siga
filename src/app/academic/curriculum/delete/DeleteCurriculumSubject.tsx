...
export const DeleteCurriculumSubject = () => {
  ...
  const handleDeleteCurriculumSubject = async () => {
    ...
    <ModalOverlay isOpen={isOpen} onClose={onClose} >
      <div className="bg-gray-50 rounded-md shadow-sm flex flex-col max-w-lg w-full" onClick={evt => evt.stopPropagation()}>
        ...
        <div className="p-4 w-full flex items-center gap-4 justify-end" >
          ...
          <button onClick={handleDeleteCurriculumSubject} className={`bg-primary rounded-md text-gray-50 px-4 py-2`}>Continuar</button>
        </div>
      </div>
    </ModalOverlay>
  )
