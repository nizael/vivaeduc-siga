...
export const ToolBar = () => {
  const { openModal } = useCurriculumStore()
  return (
    <section className="flex items-center justify-between max-sm:flex-col gap-4">
      ...
      <div className="flex items-center gap-4 max-sm:justify-between max-sm:w-full">
        <button onClick={openModal} className="border bg-[--bg-primary] text-gray-50 rounded-full  max-sm:w-full  max-sm:justify-center h-[40px] px-4 flex items-center gap-1"><b className="text-2xl">+</b> Nova grade currícular</button>
      </div>
    </section>
  )
}