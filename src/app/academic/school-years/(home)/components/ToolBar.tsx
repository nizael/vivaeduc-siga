...
export const ToolBar = () => {
  const { onOpen } = useSchoolYearCreateModalStore()
  const { toggleSequence, sequence } = useSchoolYearStore()
  return (
    <section className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={onOpen} className="border bg-[--bg-primary] text-gray-50 rounded-full h-[40px] px-4 flex items-center gap-1"><b className="text-2xl">+</b> Novo período</button>
      </div>
    </section>
  )
}