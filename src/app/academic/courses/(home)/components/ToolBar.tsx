...
export const ToolBar = () => {
  const { onOpen } = useCourseCreateModalStore()
  const { sequence, toggleSequence } = useCourseStore()
  return (
    <section className="flex items-center justify-between max-sm:flex-col gap-4">
      ...
      <div className="flex items-center gap-4 max-sm:justify-between max-sm:w-full">
        ...
        <button onClick={onOpen} className="border bg-[--bg-primary] text-gray-50 rounded-full h-[40px] px-4 flex items-center gap-1"><b className="text-2xl">+</b> Novo curso</button>
      </div>
    </section>
  )
}