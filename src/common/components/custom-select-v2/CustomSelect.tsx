    ...
    <label className={`flex flex-col gap-2 justify-between ${className}`} >
      ...
      <div className="relative h-[42px] w-full" ref={dropdownRef}>
        <input
          ...
          name={name}
          onChange={onChange}
          className="-z-10 opacity-0 max-w-[50px] absolute left-1/4 outline-none pointer-events-none"
          ref={inputRef}
        />
        <button
          type="button"
          ...
          className={`${disabled ? 'bg-gray-200' : 'bg-gray-50'
            } flex justify-between w-full border border-[#C1BBEB] rounded-lg px-1 py-2 h-11 text-left focus:outline-none focus:ring-2 focus:ring-[--bg-primary]`}
